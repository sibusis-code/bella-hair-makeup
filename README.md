...
## 6. What's Needed to Complete the Booking System

The pieces below are the gap between "mostly built" and "live and reliable." These are the
**required steps** — grouped by priority.

### A. Go-Live Blockers (must do before taking real bookings)

1. **Create the database schema.**
   `ensureSalonBookingsSchema()` only *upgrades* the `salon_bookings` table — it does **not create
   it**. The core tables (`salon_bookings`, admin users, etc.) must be created up front via a SQL
   dump / import on the production database. *Action: confirm a `schema.sql` / DB dump exists and is
   imported on the live database, and document it.*

2. **Switch PayFast from sandbox to live.**
   [.env](.env) currently has `PAYFAST_SANDBOX=true` with sandbox-style credentials. For real money:
   set `PAYFAST_SANDBOX=false`, enter the **live** `PAYFAST_MERCHANT_ID` / `PAYFAST_MERCHANT_KEY`,
   set a **passphrase** (and the same one in the PayFast dashboard), and confirm the
   **notify URL** (`https://bellahairandmakeup.co.za/itn.php`) is publicly reachable.

3. **End-to-end payment test on live.**
   Run one real (small) booking through PayFast and confirm: deposit charged → ITN received →
   booking saved as `paid` → confirmation email sent → booking visible in admin dashboard.

### B. Reliability & Operations

4. **Email deliverability.**
   `.env` sets `SEND_CLIENT_EMAILS=true` / `SEND_ADMIN_EMAILS=true` but uses PHP `mail()`
   (`EMAIL_USE_SMTP=false`). PHP `mail()` is unreliable and often lands in spam. *Action: configure
   SMTP (e.g. the `bookings@bellahairandmakeup.co.za` mailbox) for dependable confirmations.*

5. **WhatsApp confirmation (policy alignment).**
   The policy says deposit details / confirmations are sent via WhatsApp, but the system currently
   only sends email. *Decision needed: keep WhatsApp manual, or integrate a WhatsApp API for
   automatic confirmations.*

6. **Abandoned-checkout cleanup.**
   Rows in `booking_payment_attempts` that never complete payment stay as `initiated`. *Action: a
   small scheduled cleanup (or admin view) so the slot isn't perceived as taken and the table stays
   tidy.*

7. **Reminders & no-show handling.**
   Optional but high-value: automated appointment reminders (reduce no-shows) and a clear admin
   action to mark no-shows (deposit forfeited per policy).

### C. Admin & Owner Experience

8. **Admin onboarding.** Create the first admin account(s), set strong passwords, confirm the
   dashboard, reschedule, cancel and CSV export all work against live data.

9. **Calendar / day view.** Confirm the per-day slot overview gives staff a clear picture of who is
   booked with which stylist, per location.

### D. Nice-to-Haves (future)

- Customer-facing self-service reschedule/cancel (within the 48-hour policy window).
- Google Calendar sync for stylists.
- Deposit balance tracking (what's still owed at the appointment).
- Reporting: revenue by service / stylist / location.

---

## 7. Open Questions for the Business

Before building further, confirm:

1. **PayFast account** — is the live merchant account ready, and what is the passphrase?
2. **Confirmations** — email only, WhatsApp only, or both? Automated or manual WhatsApp?
3. ~~**Cash deposits** — keep the "50% cash on arrival" option, or online-deposit only?~~ **Resolved (2026-06-16): cashless — online deposit only.**
4. **Wig pricing** — public site says "price on request"; should the R1,500 base be used for
   deposits, or should wigs route to a manual quote?
5. **Reminders** — do you want automated reminders, and how long before the appointment?
6. **Who manages the back office** — how many admin users, and at which locations?

---

## 8. Summary

Bella already has a capable booking engine: a multi-service form, automatic 50% deposit
calculation, PayFast checkout, server-verified payment confirmation, double-booking protection,
and a full admin back office. The remaining work is mostly **going live and hardening** —
creating/confirming the live database schema, switching PayFast to production, making email
(and optionally WhatsApp) confirmations reliable, and running a clean end-to-end test — rather
than building from scratch.

Next step: confirm the **Section 7** answers, then work through the **Section 6 — A (Go-Live
Blockers)** checklist.
...
```
The public pages (home, services, about, booking form) render even without a database
thanks to `ALLOW_DB_OFFLINE_MODE`. Bookings, payments, and the admin back office require a
MySQL/MariaDB database with the schema from `db/migrations/` applied.

### Deploy to Vercel
This is a server-side PHP app, deployed via the community PHP runtime
(`vercel-php`, see `vercel.json`). Steps:
1. Import this GitHub repo at [vercel.com/new](https://vercel.com/new).
2. In **Settings → Environment Variables**, add the keys from `.env.example`
   (at minimum `DB_HOST` — a non-resolving value keeps the marketing pages in offline
   mode; a real external MySQL host enables bookings + admin).
3. Deploy.

**Vercel limitations (serverless):** `.htaccess` is ignored, the filesystem is read-only
(PHP sessions don't persist, so admin login is unreliable), and there is no built-in MySQL —
a fully working demo needs an **external** managed MySQL. For production, traditional PHP
hosting (e.g. cPanel/Xneelo) is the better fit.
